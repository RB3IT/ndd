...
class ResultsController(advancedtkinter.Controller):
    ...
    def loadtabs(self):
        ...
        for testname,fails in self.fail:
            ...
            tab.failurelisttreeview.bind("<<TreeviewSelect>>", lambda *e, tab = tab: self.showdoor(tab,tab.failurelisttreeview))
    ...
    def showchart(self,tab):
        ...
        baddata = [self.testdata[bad.strip()] for bad in badjobnumbers]
        con = self.addchildcontroller(ChartController, eventmanager = None, title = tab.result['title'], gooddata = gooddata, baddata = baddata)
        con.startup()

    def showdoor(self,tab,treeview):
        row = treeview.selection(1)
        if not row:
            tab.selecteddoor.config(text = "")
            return
        door =self.testdata[row] 
        tab.selecteddoor.config(text = f"{door['width']}x{door['stopheight']}")
