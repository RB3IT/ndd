...
def curtain_getendlockweight(self,slats, endlockweight, windlocks = False):
...
def curtain_getwraplength(centerlineheight, totalradius, adjustratio = 0):
    """ Gets the calculated distance for the wrap, which is the section of the curtain
        ...
        covers. By default (adjustratio of 0), .75 of the circumference of the pipe is wrapped
        (starting from the pipe's centerline in front, and wrapping around to the bottom
        of the pipe). The adjustment can be up to .75, which would result in a 0% of the 
        pipe being wrapped- ergo, the return will be identical to the centerlineheight
        supplied.
    """
    if 0 > adjustratio > .75: raise ValueError("adjustratio must be 0.0 .. 0.75 inclusive")

    return  2 * math.pi * totalradius * (.75 - adjustratio) + centerlineheight
...
def curtain_perforationsperslat(slatlength):
    ...
    return max(self.slatlength/8.5 - 2,0)
...
def pipe_centerlineheight(stopheight, upset, bracketplate_holeheight): ## HC
    ...
    """
    return stopheight + upset + bracketplate_holeheight
...
def pipe_preturns(requiredtorque_open, torqueperturn): ## PT
    ...
    """
    return requiredtorque_open / torqueperturn 
